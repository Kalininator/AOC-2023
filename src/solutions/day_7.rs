...
fn part1(input: &str) {
    ...
    //     let hand = entry.hand.chars().collect::<Vec<char>>();
    //     let hand_type = hand_type(hand);
    //     println!("{} is {:?}", entry.hand, hand_type);
    // }
    ...
    entries.sort_by(|a, b| {
        ...
        if a_hand_type == b_hand_type {
            cmp_hand_values(
                a_hand.iter().map(|c| card_char_to_value(*c)).collect::<Vec<usize>>(),
                b_hand.iter().map(|c| card_char_to_value(*c)).collect::<Vec<usize>>(),
            )
        } else {
    ...
    for (i, entry) in entries.iter().enumerate() {
        ...
        let hand_type = hand_type(entry.hand.chars().collect::<Vec<char>>());
        println!("{:?} {:?} rank {} wins {}", entry, hand_type, i + 1, winnings);
        total += winnings;
    }
    println!("{}", total);
}
